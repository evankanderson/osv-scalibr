...
func isDashboardPresent() bool {
	// Create a new HTTP request
	req, err := http.NewRequest("GET", "http://127.0.0.1:8265", nil)
	if err != nil {
...
// attempExploit attempts to exploit the vulnerability by touching a random file via HTTP query
func attempExploit() string {
	...
	statusCode := rayRequest("127.0.0.1", 8265, testCmd)
...
func rayRequest(host string, port int, cmd string) int {
